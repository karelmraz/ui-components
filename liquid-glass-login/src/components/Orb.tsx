...
// Shader adapted from ReactBits' Orb (reactbits.dev, MIT): same palette and
// noise core, ported from OGL to raw WebGL, with the petal-harmonic silhouette
// and background-luminance adaptation added on top.
...
const FRAG = /* glsl */ `
  ...
  vec4 draw(vec2 uv) {
    ...
    // Petal modulation — 6 soft lobes whose *amplitude* breathes between 0
    // (pure circle) and full (flower) over time, so the silhouette
    // continuously morphs between the two shapes.
    float petalCycle = sin(iTime * 0.8) * 0.5 + 0.5; // 0 → 1 → 0, ~8s period
    // Irregular star: stack a few incommensurate angular harmonics plus a
    // slow angular-noise term so the lobes differ in height and spacing and
    // the silhouette never repeats cleanly — no two "points" look the same.
    float petals = (
...
export function Orb({
  ...
  useEffect(() => {
    ...
    // Constant for the lifetime of this effect — parse once, not per frame.
    ...
    // Under prefers-reduced-motion the blob's ambient churn slows to a crawl
    // while hover easing stays responsive.
    ...
    const render = (t: number) => {
      ...
      lastTime = t;
      simSec += dt * (reducedMotion.matches ? 0.05 : 1);
      const sec = simSec;
